...
There are many approaches to design interval/set-membership estimators and predictors \citep{Jaulin02,Kieffer_Walter04,Bernard_Gouze04,Moisan_Bernard_Gouze09}, and this section focuses on the design based on the monotone systems theory \citep{Bernard_Gouze04,Moisan_Bernard_Gouze09,RVZ10,REZ11,Efimov2012}.
...
&M &&= 2^d\\
&\Delta A_{i}&&={h_i} \sqrt{\frac{\beta_N(\delta)}{\lambda_{\max}(G_{N,\lambda})}},& 
&h_i&&\in\{-1,1\}^d.
\end{alignat*}
...
In this section, as a first application of the estimation and prediction tools introduced above, our goal is to design a robust control $u(t)$ that stabilises \eqref{eq:dynamics}, \eqref{eq:outputs} at a vicinity of the origin under \Cref{assumpt:structure,assumpt:bounded-state,assumpt:bounded-noise}
such that
\begin{equation}
...
input $\delta(t)$ \citep{Efimov_tac2013}. In other words, it is necessary
...
also hold for such a control. Recall that we denote $\dd t>0$ the time step and $t_{n}=n\dd t$ for $n\in\Natural_{+}$, and define $T>\dd t $ as the \gls{MPC} prediction horizon, \ie at each $t_n\geq0$, to design
the input $u(t)$, an optimal control problem is solved on the interval $[t_n,t_n+T]$, and this optimal control problem is resolved again after $\dd t$ units of time (on the interval $[t_n,t_{n+1} = t_n+\dd t)$ the obtained
optimal control is applied). Then the developed \gls{MPC} algorithm can be formalized as follows for
any $t_{n}\geq0$:
\begin{enumerate}
	\item Take the confidence region $\cC_{[n],\delta}$ from \eqref{eq:confidence-ellipsoid} and calculate the matrices $A_{0}$ and $\Delta A_{i}$ with $i\in[2d]$ for \eqref{eq:polytope}.
	\item Find
	\begin{equation}
	\label{eq:OCP}
	...
	\begin{enumerate}
		...
		[y_{1}(t_{n})-\underline{\nu}_{1}(t_{n}),y_{1}(t_{n})+\overline{\nu}_{1}(t_{n})] \cap[\underline{x}(t_{n-1}+\dd t),\overline{x}(t_{n-1}+\dd t)], & n\geq1\\
		\end{cases}\nonumber
		\end{align}
		where $(\underline{x}(t_{n-1}+\dd t)^{\top},\overline{x}(t_{n-1}+\dd t)^{\top})^{\top}=\xi(t_{n-1}+\dd t)$
...
\begin{proof}
	...
	result of \Cref{thm:confidence_ellipsoid}, and the size of the set $\hat{\Theta}(t)$
	...
	According to the definition \eqref{eq:X_f} of $\X_{f}$ and \Cref{assumpt:feasible-constr}, $\xi(t)\in\X_{f}$ and $u(t)\in\U$ for all $t\geq t_{k}$,
...
%	
%	In both experiments, we used $\discount=0.9$,  $\delta=0.9$ and a planning budget $K=100$. The perturbations were sampled uniformly in $[-0.1, 0.1]^r$ while the measurements are Gaussian with covariance $\Sigma_r = 0.1 I_s$. 
%	
