...
\hlg{\sup_{\bu\in{(\Real^q)}^\Natural}} \underbrace{\hlr{\inf_{\substack{\theta \in \confidenceset \\ \omega\in[\underline{\omega},\overline{\omega}]^\Natural}}} \left[\sum_{n=N+1}^\infty \gamma^n R(x_n(\bu,\omega))\right]}_{{V^r(\bu)}},
\end{equation}
where $x_n(\bu,\omega)$ is the state reached at step $n$ under controls $\bu$ and perturbations $\omega$ within the given admissible bounds $[\underline\omega(t),\overline\omega(t)]$, and $\R$ is an \emph{arbitrary} bounded reward function. This choice of rich reward space is crucial to have sufficient flexibility to model non-convex and non-smooth functions that naturally arise in many practical problems involving combinatorial optimisation, branching decisions, etc., while quadratic costs are mostly suited for tracking a fixed reference trajectory \citep[\eg][]{Kumar2013}.
...
\begin{assumption}[Bounded disturbance]
	...
	At each time $t\geq0$, the disturbance $\omega(t)$ is enclosed by \emph{known} signals $\underline{\omega}(t) \leq \omega(t) \leq \overline{\omega}(t)$, whose amplitude verifies $\sum_{n=0}^\infty \gamma^n C_\omega(t_n) < \infty$, where $$C_\omega(t) \eqdef \sup_{\tau\in[0,t]} \|\overline{\omega}(\tau) - \underline{\omega}(\tau)\|_2.$$
\end{assumption}

\begin{assumption}[Sub-Gaussian observations]
	\label{assumpt:gaussian-noise}
	At each time $t\geq0$, the combined noise $\eta(t)$ is an independent sub-Gaussian noise with covariance proxy $\Sigma_p \in \Real^{p\times p}$:
	$$
	\forall u\in\Real^p,\; \expectedvalue \left[ \exp{\left( u^\transp \eta(t)\right)}\right] \leq \exp{\left( \frac{1}{2} u^\transp \Sigma_p u\right)}.
	$$
\end{assumption}

Under these assumptions, we can derive a confidence ellipsoid for $\params$.
...
\begin{definition}[Hurwitz]
	\begin{leftbar}[defnbar]
		A matrix $A\in\Real^{n\times n}$ is called \emph{Hurwitz} if all its eigenvalues have negative real parts.
	\end{leftbar}
\end{definition}
\begin{definition}[Metzler]
	\begin{leftbar}[defnbar]
		A matrix $A\in\Real^{n\times n}$ is called \emph{Metzler} if all its elements outside the main diagonal are nonnegative.
	\end{leftbar}
\end{definition}
...
\begin{align}
\label{eq:polytope}
&\cP = \left\{ A_{N}+\sum_{i=1}^{M}\lambda_{i}\Delta A_{N,i}: \lambda\geq 0,  \sum_{i=1}^{M}\lambda_{i}=1\right\},
\end{align}
where 
\begin{alignat*}{4}
&A_{N}&&=A + \theta_{N,\lambda}^\transp\phi, & \hspace{4em}
&M &&= 2^d\\
&\Delta A_{N,i}&&= \sqrt{\frac{\beta_N(\confidence)}{\lambda_{\max}(G_{N,\lambda})}}{h_i}^\top\phi,& 
&h_i&&\in\{-1,1\}^d.
\end{alignat*}
...
\begin{assumption}
	...
	\end{leftbar}
\end{assumption}
In practice, this assumption is often verified. It is for instance the case whenever $A_{0}$ is diagonalizable, or a method from \citep{Efimov2013} computes a similarity transformation $Z$ when the system is observable with respect to a scalar output. To simplify the notation, we further assume that $Z=I_{p}$ so that the system \eqref{eq:LPV_syst} has already been put in the right form:
\[
...
\begin{equation}
\safestates_{f}=\{\xi\in\Real^{2p}:V(\xi)\leq\alpha^{-1}\sup_{t\geq0}|\tilde{\delta}^{\top}(t)\Gamma\tilde{\delta}(t)|\},\label{eq:X_f}
\end{equation}
...
\begin{assumption}
	...
	\[
	K_{0}\xi+K_{1}\xi^{+}+K_{2}\xi^{-}+S\delta(t)\in\safecontrols
	\]
...
\begin{equation}
\hlg{\sup_{bu\in{(\Real^q)}^\Natural}} \hlr{\inf_{\substack{\theta \in \confidenceset \\ \omega\in[\underline{\omega},\overline{\omega}]^\Real}}} \left[\sum_{n=N+1}^\infty \gamma^n R(x_n(\bu,\omega))\right]. \tag{\ref{eq:minimax-obj}}
\end{equation}
...
\end{proof}
A direct consequence of \Cref{prop:lower-bound} is that since all our approximations are conservative, if we manage to find a control sequence such that no \emph{\enquote{bad event}} (\eg a collision) happens according to the surrogate objective $\hat{V}^r$, then we are guaranteed that they will not happen either when the controls are executed on the true system.
...
where $x_n(\ba, \omega)$ stems from \eqref{eq:dynamics} with $u_n = \pi_{a_n}(x_n)$.
...
\begin{lemma}[Planning performance of \citealp{Hren2008}]
...
\begin{theorem}[Regret bound]
	...
	\begin{equation*}
	V(a^\star) - \hat{V}^r(a_K) \leq \hlr{\underbrace{\Delta_\omega}_{\substack{\text{robustness to}\\ \text{perturbations}}}} + \hlb{\underbrace{\cO\left(\frac{\beta_N(\delta)^2}{\lambda_{\min}(G_{N,\lambda})}\right)}_{\text{estimation error}}} + \hlg{\underbrace{\cO\left(K^{-\frac{\log 1/\gamma}{\log \kappa}}\right)}_{\text{planning error}}} 
	\end{equation*}
	with probability at least $1-\confidence$, where $V(a)$ is the optimal expected return when executing an action $a\in\cA$, $a_\star$ is an optimal action, and $\hlr{\Delta_\omega}$ is a constant which corresponds to an irreducible suboptimality suffered from being robust to instantaneous disturbances $\omega(t)$.
	\end{leftbar}
\end{theorem}
\begin{proof}
...
\begin{corollary}[Asymptotic near-optimality]
	...
	\end{align}
	the stability condition 2. of \Cref{thm:minimax-regret-bound} can be relaxed to apply to the true system: there exist $P,Q_0,\rho$ such that
	$$\begin{bmatrix}
	A(\theta)^\transp P + P A(\theta) + Q_0 & P|D|  \\
	|D|^\transp P & -\rho I_r \\
	\end{bmatrix}< 0;$$
	and the regret bound in \Cref{thm:minimax-regret-bound} takes the more explicit form:
	\begin{equation*}
	V(a_\star) - \hat{V}^r(a_K) \leq \hlr{\underbrace{\Delta_\omega}_{\substack{\text{robustness to}\\ \text{perturbations}}}} + \hlb{\underbrace{\cO\left(\frac{\log\left(N^{d/2}/\delta\right)}{N}\right)}_{\text{estimation error}}} + \hlg{\underbrace{\cO\left(K^{-\frac{\log 1/\gamma}{\log \kappa}}\right)}_{\text{planning error}}} 
	\end{equation*}
...
\begin{figure}[tp]
	...
	\caption{The mean (solid), $95\%$ confidence interval for the mean (shaded) and maximum (dashed) simple regret with respect to $N$.}
	\label{fig:regret}
\end{figure}

\begin{table}[tbp]
	\caption{Performances on the obstacle task. We give the frequency of collision, minimum and average return achieved on a single episode, repeated with 100 random seeds. The robust agent performs worse than the nominal agent on average, but manages to ensure safety and attains a better worst-case performance.}
	\label{tab:obstacle}
	\centering
	\begin{tabular}{lccc}
		\toprule
...
\begin{table}[t]
	\caption{Performances on the driving task. We make the same observations as in \Cref{tab:obstacle}.}
	\label{tab:driving}
	\centering
	\begin{tabular}{lccc}
		\toprule
		...
		\midrule
		\gls{DQN} (trained) & 3\% & 5.4 & $6.3\pm0.6$ \\
		\bottomrule
	\end{tabular}
\end{table}
...
%\begin{subappendices}
%\input{2-Chapters/7-Chapter/appendix7}
%\end{subappendices}
