...
%	Assuming that \eqref{eq:polytope} and \Cref{assumpt:metzler} are satisfied for the system \eqref{eq:dynamics}, then the interval predictor
%	\begin{eqnarray}
...
For the sake of generality, we consider a slightly more general setting that \Cref{assumpt:structure}: rather than specifying a linear relationship $A(\theta)$ (which was needed for estimation), we only require it to lie within a known polytope.
