...
\begin{definition}[Budgeted Optimality]
	...
	\begin{enumerate}[label=(\roman*)]
		...
		\end{equation}
		\item the optimal value function for rewards $\Vr^*$ and candidate policies $\policies_r$ as
		\begin{equation}
		...
		\end{equation}
		\item the optimal value function for costs $\Vc^*$ and optimal policies $\policies^\star$ as
		\begin{equation}
		...
		\end{equation}
	\end{enumerate}
	We define the budgeted action-value function $\oQ^*$ similarly as
	\begin{equation}
...
\begin{remark}[Appearance of the greedy policy]
	...
	\label{rmk:greedy}
	In classical \acrlong{RL}, the greedy policy takes a simple form $\policy_\text{greedy}(s; \Q^*) = \argmax_{a\in\cA} \Q^*(s, a)$, and the term $\policy_\text{greedy}(a'|s';\Q^*) \Q^{*}(s', a')$ in \eqref{eq:bellman-optimality} conveniently simplifies to $\max_{a'\in \cA} \Q^*(s', a')$. Unfortunately, in a budgeted setting the greedy policy requires solving the nested constrained optimisation program \eqref{eq:pi_greedy} at each state and budget in order to apply this Budgeted Bellman Optimality operator.
	\end{leftbar}
\end{remark}
...
\begin{proposition}[Optimality of the greedy policy]
	...
	The greedy policy $\budgetedpolicy_\text{greedy}(\cdot~; \oQ^*)$ is \emph{uniformly} optimal: for all $\os\in\ocS$, $\budgetedpolicy_\text{greedy}(\cdot~; \oQ^*)\in\policies^*(\os)$. In particular, $\oV^{\budgetedpolicy_\text{greedy}(\cdot; \oQ^*)} = \oV^*$ and $\oQ^{\budgetedpolicy_\text{greedy}(\cdot; \oQ^*)}= \oQ^*$.
	\end{leftbar}
\end{proposition}
...
The Budgeted Bellman Optimality equation is a fixed-point equation, which motivates the introduction of a fixed-point iteration procedure. We introduce \Cref{algo:bvi}, a \acrlong{DP} algorithm for solving known \glspl{BMDP}. If it were to converge to a unique fixed point, this algorithm would provide a way to compute $\oQ^*$ and recover the associated optimal budgeted policy $\budgetedpolicy_\text{greedy}(\cdot~; \oQ^*)$.

\begin{theorem}[Non-contractivity of $\abo^{*}$]
	\begin{leftbar}[theorembar]
	\label{thm:contraction}
	For any \gls{BMDP} ($\cS,\cA,\Ps,\R,\constraint,\discountfactor$) with $|\cA| \geq 2$, $\abo^{*}$ is not a contraction. Precisely: $\forall\epsilon>0, \exists Q^1,Q^2\in(\Real^2)^{\ocS\ocA}:\|\cT Q^1-\cT Q^2\|_\infty \geq \frac{1}{\epsilon}\|Q^1-Q^2\|_\infty$.
	\end{leftbar}
\end{theorem}

Unfortunately, as $\abo^{*}$ is not a contraction, we can guarantee neither the convergence of \Cref{algo:bvi} nor the unicity of its fixed points. Despite those theoretical limitations, we empirically observed the convergence to a fixed point in our experiments (\Cref{sec:brl-experiments}). We provide a possible explanation:

\begin{theorem}[Contractivity of $\cT$ on smooth $Q$-functions]
	\begin{leftbar}[theorembar]
	\label{rmk:contractivity-smooth}
	The operator $\abo^{*}$ is a contraction when restricted to the subset $\cL_{\discountfactor}$ of $\oQ$-functions such that "$\Qr$ is Lipschitz with respect to $\Qc$":
	\begin{equation}
...
Thus, we expect that \Cref{algo:bvi} is likely to converge when $\oQ^*$ is smooth, but could diverge if the slope of $\oQ^*$ is too high.  $L^2$-regularisation can be used to encourage smoothness and mitigate risk of divergence.
...
When the \gls{BMDP} is unknown, we need to adapt \Cref{algo:bvi} to work with a batch of samples $\cD=\{(\os_t,\oa_t,\overline{r}_t,\os_t')\}_{t\in [1,N]}$ collected by interaction with the environment. Applying $\abo^{*}$ in \eqref{eq:bellman-optimality} would require computing an expectation $\expectedvalue_{\os'\sim \augmentedtransition}$ over next states $\os'$ and hence an access to the \idx{model} $\augmentedtransition$. We instead use $\hat{\abo^{*}}$, a sampling operator, in which this expectation is replaced by
\begin{equation*}
\hat{\abo^{*}} \oQ(\os, \oa, \overline{r}, \os') \eqdef \overline{r} + \discount \sum_{\ov{a'}\in \ocA} \pi_\text{greedy}(\ov{a'}|\ov{s'}; \oQ) \oQ(\ov{s'}, \ov{a'}).
\end{equation*}
...
Any model can be used, such as linear models, regression trees, or \acrlong{NN}.
...
\begin{figure}[tp]
	...
	\caption{Calibration of a penalty multiplier according to the budget $\budget$. The optimal multiplier $\lambda^*_{\text{avg}}$ is the smallest one to satisfy the budget constraint on average. Safer policies can also be selected according to the largest deviation from this mean cost.}
...
In our third application, we use the \href{https://github.com/eleurent/highway-env}{highway-env} \idx{environment} \citep{highway-env} for simulated highway driving and behavioural decision-making.
...
The task consists in driving as fast as possible, which is \idx{model}led by a reward proportional to the velocity: $\R(s_t, a_t) \propto v_t$. This motivates the \idx{agent} to try and overtake its preceding vehicles by driving fast on the opposite lane. This optimal but overly aggressive behaviour can be tempered through a cost function that embodies a safety objective: $\constraint(s_t, a_t)$ is set to $1/H$ whenever the ego-vehicle is driving on the opposite lane, where $H$ is the trajectory\index{trajectory} horizon. Thus, the constrained signal is the maximum proportion of time that the \idx{agent} is allowed to drive on the wrong side of the road.
...
Algorithms have been proposed for \glspl{CMDP}, which are less flexible sub-problems of the more general \gls{BMDP}. When the \idx{environment} parameters ($\Ps$, $\R$, $\constraint$) are known but not tractable, solutions relying on function approximation~\citep{Undurti} or approximate linear programming~\citep{Poupart2015} have been proposed. For unknown \idx{environment}s, \idx{Online} algorithms \citep{Geibel2005, Abe2010,AchiamHTA17,ChowGJP15} and a \idx{batch} algorithm \citep{Thomas2015, Petrik2016, Laroche2019,Le2019} can solve large unknown \glspl{CMDP}. Nevertheless, these approaches are limited in that the constraints thresholds are fixed prior to training and cannot be updated in real-time at policy execution to select the desired level of risk.
...
\begin{proof}
	...
	\end{align*}
	\begin{align*}
	Q^\pi(\os, \oa) &\eqdef \expectedvalue\left[\sum_{t=0}^\infty \discount^t R(\os_t, \oa_t)\condbar \ov{s_0} = \os, \ov{a_0} = \oa\right] \\
	&= R(\os, \oa) + \sum_{\os'\in\ocS}\probability{\os_1 = \os' \condbar\ov{s_0} = \os, \ov{a_0} = \oa}\cdot \expectedvalue\left[\sum_{t=1}^\infty \discount^t R(\os_t, \oa_t)\condbar \ov{s_1} = \os'\right] \\
	&= R(\os, \oa) + \discount\sum_{\os'\in\ocS}\ov{P}\left(\os' \condbar\os, \oa\right) \expectedvalue\left[\sum_{t=0}^\infty \discount^t R(\os_t, \oa_t) \condbar \ov{s_0} = \os'\right] \\
	&=  R(\os, \oa) + \discount\sum_{\os'\in\ocS}\ov{P}\left(\os' \condbar\os, \oa\right) V^\pi(\os')
	\end{align*}
	
	\textbf{Contraction of $\cT^\pi$:}
	Let $\pi\in\Pi, Q_1, Q_2\in(\Real^2)^{\ocS\ocA}$.
	\begin{align*}
	\forall \os\in\ocS, \oa\in\ocA,\quad \left|\cT^\pi Q_1(\os,\oa) - \cT^\pi Q_2(\os,\oa)\right| &= \left|\discount\expectedvalueover{\substack{\os'\sim\ov{P}(\os'|\os,\oa) \\ \oa'\sim\pi(\oa'|\os')}} Q_1(\os',\oa') - Q_2(\os',\oa')\right|\\
	&\leq \discount\left\|Q_1-Q_2\right\|_\infty
	\end{align*}
	Hence, $\left\|\cT^\pi Q_1  - \cT^\pi Q_2 \right\|_\infty \leq \discount\left\|Q_1-Q_2\right\|_\infty$
...
\begin{proof}
	...
	&= \max_{\rho, \pi'} R_r(s, a) + \discount \sum_{\os'\in\ocS} P(\os' | \os, \oa) V_r^{\rho, \pi'}(\os') \label{eq:pthm_exp}\\
	&= R_r(s, a) + \discount \sum_{\os'\in\ocS}  P(\os' | \os, \oa) \max_{\rho, \pi'} \sum_{\oa'\in\ocA} \rho(\oa' | \os')Q_r^{\pi'}(\os', \oa') \label{eq:pthm_marg}\\
	&= R_r(s, a) + \discount \sum_{\os'\in\ocS}  P(\os' | \os, \oa) \max_\rho\sum_{\oa'\in\ocA}\rho(\oa' | \os')\max_{\pi'\in\Pi_a(\os')}Q_r^{\pi'}(\os', \oa') \label{eq:pthm_max}\\
	&= R_r(s, a) + \discount \sum_{\os'\in\ocS}  P(\os' | \os, \oa) \max_\rho\expectedvalueover{\oa'\sim\rho}Q_r^*(\os', \oa') \label{eq:pthm_marg_def2}
	\end{align}
	...
	\begin{equation*}
	Q_r^*(\os, \oa) = r(\os, \oa) + \discount \sum_{\os'\in\cS}  P(\os' | \os, \oa) \expectedvalueover{\oa'\sim\pi_\text{greedy}(\os', Q^*)} Q_r^*(\os', \oa')
	\end{equation*}
...
\begin{proof}
	...
	\begin{align*}
	\|\cT Q_\epsilon^1(\os, \oa) - \cT Q_\epsilon^2(\os, \oa)\|_\infty &= \discount\left\|\expectedvalueover{\os'\sim\ov{P}(\os'|\os,\oa)} \expectedvalueover{\oa'\sim\pi_\text{greedy}(Q^1_\epsilon)}Q^1_\epsilon(\os',\oa') - \expectedvalueover{\oa'\sim\pi_\text{greedy}(Q^2_\epsilon)}Q^2_\epsilon(\os',\oa')\right\|_\infty \\
	&= \discount\left\|\expectedvalueover{\os'\sim\ov{P}(\os'|\os,\oa)}\left(\frac{1}{\discount}, \epsilon\right) - (0, \epsilon)\right\|_\infty \\
...
\begin{proof}
	We now study the contractivity of $\cT$ when restricted to the functions of $\cL_\discount$ defined as follows:
	\begin{equation}
	\cL_\discount = \left\{\begin{array}{cc}
	Q\in(\Real^2)^{\ocS\ocA}\text{ s.t. }\exists L<\frac{1}{\discount}-1: \forall \os\in\ocS,\oa_1,\oa_2\in\ocA,   \\
	|Q_r(\os,\oa_1) - Q_r(\os,\oa_2)| \leq L|Q_c(\os,\oa_1) - Q_c(\os,\oa_2)|
	\end{array}\right\}
	\end{equation}
	That is, for all state $\os$, the set $Q(\os, \ocA)$ plot in the $(Q_c,Q_r)$ plane must be the \emph{graph} of a $L$-Lipschitz function, with $L<1/\discount-1$.
	...
	This proof makes use of insights detailed in the proof of \Cref{prop:bftq_pi_hull}, which we recommend the reader to consult first.
	...
	We give the three main steps required to show that $\cT$ restricted to $\cL_\discount$ is a contraction. Given $Q^1, Q^2\in\cL_\discount$, show that
	\begin{enumerate}
		...
		\item $Q\in\cL_\discount \implies \cF$ is the graph of a $L$-Lipschitz function, $\forall\os\in\ocS$.
	...
	\textbf{Step 2:} We want to show that if $Q\in\cL_\discount$, $\cF$ is the graph of an $L$-Lipschitz function:
	\begin{equation}
	\label{eq:L-lip-set}
	\forall q^1,q^2\in\cF, |q_r^2-q_r^1| \leq |q_c^2-q_c^1|
	\end{equation}
	
	Let $Q\in\cL_\discount$ and $\os\in\ocS$, $\cF$ the corresponding top frontier of convex hull.
	...
	Finally, consider the edge case in \Cref{sec:proof_pi_hull}: the constraint is not active, and the optimal value is simply $\argmax_{q\in\cF} q^r$. In particular, since we showed that $\cF^2\in B(\cF^1, R)$, and since $Q^{2*}\in \cF^2$, there exist $q^1\in \cF^1: \|Q^{2*}-q^1\|_\infty\leq R$ and in particular $Q^{1*}_r \geq q^1_r \geq Q^{2*}_r - R$. Reciprocally, by the same reasoning, $Q^{2*}_r \geq Q^{1*}_r - R$. Hence, we have that $| Q_r^{2*} - Q_r^{1*}| \leq R \leq R(L+1).$
	
	\textbf{Wrapping it up:}
	
	We've shown that for any $Q^1,Q^2\in\cL_\discount$, and all $\os\in\ocS$, $\cF^2\in\cB(\cF^1,\|Q^2-Q^1\|_\infty)$ and $\cF^1$ is the graph of a $L$-Lipschitz function with $L<1/\discount - 1$. Moreover, the solutions of $\pi_\text{greedy}(Q^1)$ and $\pi_\text{greedy}(Q^2)$ at $\os$ are such that $ \|Q^{2*} - Q^{1*}\|_\infty \leq (L+1)\|Q^2-Q^1\|_\infty$.
	
	Hence, for all $\oa$,
	\begin{align*}
	\|\cT Q^1(\os, \oa) - \cT Q^2(\os, \oa)\|_\infty &= \discount\left\|\expectedvalueover{\os'\sim\ov{P}(\os'|\os,\oa)} \expectedvalueover{\oa'\sim\pi_\text{greedy}(Q^1)}Q^1(\os',\oa') - \expectedvalueover{\oa'\sim\pi_\text{greedy}(Q^2)}Q^2(\os',\oa')\right\|_\infty \\
	&= \discount\left\|Q^{2*} - Q^{1*}\right\|_\infty \\
	&\leq \discount(L+1)\|Q^2-Q^1\|_\infty
	\end{align*}
	Taking the sup on $\ocS\ocA$,
	
	\begin{equation*}
	\|\cT Q^1 - \cT Q^2\|_\infty \leq \discount(L+1)\|Q^1-Q^2\|_\infty
	\end{equation*}
	with $\discount(L+1) < 1$.
	As a conclusion, $\cT$ is a $\discount(L+1)$-contraction on $\cL_\discount$.
\end{proof}
...
%Consequently, $V_r^*(s', \cdot)$ is non-decreasing and concave. By \eqref{eq:bellman_expectation_Q} we see that $Q_r^*(s,a,\cdot) = R_r(s,a) + \discount\expectedvalueover{s'}V_r^*(s', \cdot)$  is too.
