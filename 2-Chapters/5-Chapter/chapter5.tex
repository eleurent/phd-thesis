...
\begin{table}[H]
	\begin{tabularx}{\linewidth}{lcX}
		\toprule
		Criterion & Meet & Description \\
		\midrule
		\textbf{Social Awareness} & {\Large $\hlgb{\checkmark}$} & using the ego-attention architecture to actively attend to other vehicles \\
		\textbf{Sample Efficiency} & {\Large $\hlgb{\checkmark}$} & social-awareness is an inductive bias wich fastens training \\
		\textbf{Safety} & {\Large $\hlgb{\checkmark}$} & a cost signal is constrained to remain below a threshold budget \\
		\textbf{Balance between safety and efficiency} & {\Large $\hlgb{\checkmark}$} & the budget beta can be adjusted in real time as an input to the budgeted policy to trade-off safety with/ efficiency \\
		\bottomrule
	\end{tabularx}
\end{table}