...
\abstractStartChapter{}%
  ...
  Our approach is motivated by the prospect of training a risk-sensitive driving policy for a two-way road, where overtaking a vehicle requires driving on the wrong lane.\footnote{The work in this chapter has been done in collaboration with Nicolas Carrara, who came up with the algorithm and carried-out the dialogue experiment. I did most of the theoretical analysis, and the driving experiment. We worked together on the exploration procedure, part of the analysis, and scaling-up the implementation.}
\minitocStartChapter{}

\section{Reconciling efficiency and safety}
...
\begin{subfigure}[t]{0.48\linewidth}
	...
	\caption{In a \glsxtrshort{CMDP}, we learn a single policy $\pi^\star$ (blue dot) with a fixed expected risk $\budget\in\budgetspace$}
	\label{fig:cmdp}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
	\includegraphics[width=\linewidth]{img/pareto3}
	\caption{In a \glsxtrshort{BMDP}, we learn the Pareto front $\Pi^\star$ (blue frontier) for the whole set $\budgetspace$ of allowed risks}
	\label{fig:bmdp}
\end{subfigure}
...
\end{proposition}
\begin{proof}
	We provide the proof in \Cref{sec:proof-bell-eval}.
\end{proof}
...
\begin{proposition}[Equality of $\budgetedpolicy_\text{greedy}$ and $\budgetedpolicy_\text{hull}$]
	...
	The greedy policy $\budgetedpolicy_\text{greedy}$ is equal to a convex-hull policy $\budgetedpolicy_\text{hull}$ defined in \Cref{algo:pi_hull}:
    \begin{equation*}
        \budgetedpolicy_\text{greedy}(\oa|\os; \oQ) = \budgetedpolicy_\text{hull}(\oa|\os; \oQ).
    \end{equation*}
    Thus, \Cref{algo:bvi} and \Cref{algo:bftq} can be run by replacing $\budgetedpolicy_\text{greedy}$ in the equation \eqref{eq:bellman-optimality} of $\abo$ with $\budgetedpolicy_\text{hull}$.
    \end{leftbar}
\end{proposition}
\begin{proof}
...
Algorithms have been proposed for \glspl{CMDP}, which are less flexible sub-problems of the more general \gls{BMDP}. When the environment parameters ($\Ps$, $\R$, $\constraint$) are known but not tractable, solutions relying on function approximation~\citep{Undurti} or approximate linear programming~\citep{Poupart2015} have been proposed. For unknown environments, Online algorithms \citep{Geibel2005, Abe2010,Achiam2017,ChowGJP15} and a batch algorithm \citep{Thomas2015, Petrik2016, Laroche2019,Le2019} can solve large unknown \glspl{CMDP}. Nevertheless, these approaches are limited in that the constraints thresholds are fixed prior to training and cannot be updated in real-time at policy execution to select the desired level of risk.
