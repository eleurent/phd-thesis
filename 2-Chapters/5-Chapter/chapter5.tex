...
\begin{table}[H]
	\begin{tabularx}{\linewidth}{lcX}
		...
		\midrule
		\textbf{Social Awareness} & $\hlr{\checkmark}$ & using the ego-attention architecture to actively attend to other vehicles \\
		\textbf{Sample Efficiency} & $\hlr{\checkmark}$ & social-awareness is an inductive bias wich fastens training \\
		\textbf{Safety} & $\hlr{\checkmark}$ & a cost signal is constrained to remain below a threshold budget \\
		\textbf{Balance between safety and efficiency} & $\hlr{\checkmark}$ & the budget beta can be adjusted in real time as an input to the budgeted policy to tradeoff safety with/ efficiency
		\bottomrule
	\end{tabularx}
\end{table}