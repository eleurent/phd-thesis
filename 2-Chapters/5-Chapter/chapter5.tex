...
\begin{proposition}[Budgeted Bellman Expectation]
	...
	\end{align}
	Moreover, consider the Budgeted Bellman Evaluation\index{Bellman Evaluation} operator $\abo^{\budgetedpolicy}$:
...
\begin{definition}[Budgeted Optimality]
	...
	\begin{enumerate}[label=(\roman*)]
		...
		\begin{equation}
		\Vc[^\star](\os) \eqdef \min_{\budgetedpolicy\in\policies_r(\os)}  \Vc[^\budgetedpolicy](\os), \qquad\qquad \policies^*(\os) \eqdef \argmin_{\budgetedpolicy\in\policies_r(\os)}  \Vc[^\budgetedpolicy](\os).
		\end{equation}
	\end{enumerate}
	...
	\end{equation}
	and denote $\oV^* = (\Vr[^\star], \Vc[^\star])$, $\oQ^* = (\Qr[^\star], \Qc[^\star])$.
	\end{leftbar}
\end{definition}
...
When the \gls{BMDP} is unknown, we need to adapt \Cref{algo:bvi} to work with a batch of samples $\cD=\{(\os_t,\oa_t,\overline{r}_t,\os_t')\}_{t\in [1,N]}$ collected by interaction with the environment. Applying $\abo^{*}$ in \eqref{eq:bellman-optimality} would require computing an expectation $\expectedvalue_{\os'\sim \augmentedtransition}$ over next states $\os'$ and hence an access to the model $\augmentedtransition$. We instead use $\hat{\abo^{*}}$, a sampling operator, in which this expectation is replaced by
\begin{equation*}
...
% TODO: \Qr[^\star] must be replaced by \Qr[^*], same for Qc
...
Let $\os=(s,\budget)\in\ocS$ and $\oQ\in(\Real^2)^{\ocS\ocA}$. We recall the definition of $\budgetedpolicy_\text{greedy}$:
    ...
    Because $\cF$ is the graph of a non-decreasing function, $\lambda$ should be as high as possible, as long as the budget $q^*\leq\budget$ is respected. We reach the highest $q_r^*$ when $q^*_c=\budget$, that is: $\lambda=(\budget-q_c^1)/(q_c^2-q_c^1)$.
