...
Value-based \acl*{RL} algorithms such as \ac{DQN} \citep{Mnih2015} and its variants rely on an estimate of the optimal state-action value function $Q^\star$. Since the state space chosen in \Cref{chapter:3} is continuous, we must resort to function approximation. Thus, independently of how the state space $\cS$ was defined, the choice of how a state will be \textit{represented} as an input to the model plays an important part in the design of the system.
...
\TODO{Value-based algorithms vs policy gradient}
