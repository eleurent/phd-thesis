...
Value-based \acl*{RL} algorithms such as Q-Learning \citep{Watkins1992} and its variants rely on an estimate of the optimal state-action value function $Q^\star$. Since the state space chosen in \Cref{chapter:3} is continuous, we must resort to function approximation. Thus, independently of how the state space $\cS$ was defined, the choice of how a state will be \textit{represented} as an input to the model plays an important part in the design of the system. In particular, we claim that the two most-widely used representations both suffer from different drawbacks: on the one hand, the \emph{list of features} representation is compact and accurate but has a varying-size and depends on the choice of ordering. On the other hand, the \emph{spatial grid} representation addresses these concerns but in return suffers from an accuracy-size trade-off.
...
\subsection{Value-based deep \acl*{RL}}
...
\begin{proposition}[Optimality of the greedy policy, \citealt{Bellman56}]
	...
	\begin{equation*}
	\forall s\in\cS, \pi^\star\parentheses{\cdot \mid s} = \delta_{a^\star},\,\text{where } {a^\star}\in \argmax_a Q^\star(s, a),
	\end{equation*}
...
The attention architecture was introduced to enable neural networks to discover interdepen\-dencies within a variable number of inputs.
