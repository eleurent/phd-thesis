...
We start by recalling standard model-free learning algorithms, with a focus on value-based methods. Recall from \Cref{def:optimality} that $Q^\star$ can be computed from an optimal policy $\optimalpolicy$ since $Q^\star = Q^{\optimalpolicy}$. However, the converse that $\optimalpolicy$ can be obtained from knowing $Q^\star$ is also true since
\begin{proposition}[Optimality of the greedy policy, \citealt{Bellman56}]
	\begin{leftbar}[propositionbar]
	A greedy policy defined as
	\begin{equation*}
	\forall s\in\cS, \optimalpolicy\parentheses{\cdot \mid s} = \delta_{a^\star},\,\text{where } {a^\star}\in \argmax_a Q^\star(s, a),
	\end{equation*}
...
\begin{equation*}
Q^\star(s, a) = (\cT Q^*) (s, a) \eqdef \expectedvalueover{s'\sim P(s'|s, a)} \max_{a'\in \cA} \left[R(s, a) + \discount Q^\star(s', a')\right].
\end{equation*}
Moreover, $\cT$ is a $\discount$-contraction.
\end{leftbar}
\end{theorem}
