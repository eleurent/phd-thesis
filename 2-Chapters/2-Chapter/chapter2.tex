...
Another approach is to formulate safety as a constraint. Typically, that some cost function $g$ of the state stays below a given threshold in expectation. \citep{Berkenkamp2016} apply this idea to blackbox optimisation and enforce the constraint at all time using Gaussian Process regression. This idea is extended to a sequential decision-making in the \ac{CMDP} framework \citep{Altman1999,Achiam2017}, a variant of \acp*{MDP} augmented with a cost function $R^c$, and such that the return maximisation is subjected to a constraint on the maximum expected cumulative discounted cost:

\begin{equation*}
\max_\pi \expectedvalue_{\pi, P} \left[\sum_t \gamma^t R^r_t \right] \text{ subject to } \expectedvalue_{\pi, P} \left[\sum_t \gamma^t R^c_t \right] \leq \beta.
\end{equation*}

In \citep{Tessler2019}: general constraints (discounted sums or average cost settings)
%In \citep{Chow2018} constraint on percentile, with policy gradient
Applied to AD in \citep{Le2019}: two behavioural constraints: smooth driving and lane centering on CarRacing
...
\citep{Geibel2005}: Markov Decision Processes (MDPs) with error states
