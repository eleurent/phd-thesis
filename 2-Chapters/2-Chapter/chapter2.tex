...
\begin{figure}[ht]
	\centering
	\includegraphics[trim={0 0.5cm 0 0}, clip, width=0.5\linewidth]{img/roundabout}
...
\begin{equation*}
\max_\pi \min_{P\in\cP} \expectedvalue_{\pi, P} \left[G_P^\pi\right]
\end{equation*}
...
\begin{equation*}
\max_\pi \expectedvalue_{\pi, P} \left[\sum_t \gamma^t R(s_t, a_t) \right] \text{ subject to } \expectedvalue_{\pi, P} \left[\sum_t \gamma^t C(s_t, a_t) \right] \leq \beta.
\end{equation*}

Extensions have also been proposed to this framework: in \citep{Tessler2019}, more general constraints than discounted sums only are considered, while in \citep{Geibel2005,Chow2018} the constraint does concern the expectation of the discounted total cost, but rather its percentile or \ac*{CVaR}. In \citep{Le2019}, \acp*{CMDP} are applied in a car racing simulation to enforce two behavioural constraints of smooth driving (expected number of braking actions) and lane tracking (expected distance to the lane center).


\subsection{Safe Exploration}

The three paradigms mentioned so far aim to revisit the definition of optimality. However, these adjustments only alter the optimal policy once it is learnt, but they leave the exploration process subject to failures. Consequently, other approaches have been developed to enforce safety constraints at all time. The most common formalisation consists in preventing the agent from visiting any \emph{error state} during exploration-- or equivalently to remain within a set $\mathbb{X}$ of \emph{safe states} --by relying on prior knowledge such as expert demonstrations or an initial feasible policy.
...
When the system is fully known, a general solution to this problem is provided by the \ac{HJI} reachability equation, a differential game that allows to handle general constraints $\mathbb{X}$ \citep{leung2018infusing,Fisac2019}.

When the error states are defined through a level-set of safety function  $r(s,a) < h$ whose smoothness is known, \citep{Turchetta2016} propose an algorithm that explores the maximum reachable safe set without visiting unsafe states.


But often intractable.
Common example: robust constraint satisfaction with linear systems.
...
This setting is questionable for AD, might be too restrictive: some standard states are prone to errors under adversarial behaviours, and cannot be avoided. But avoiding these states altogether is also unacceptable.

We cannot always avoid this region of the state-space.
\begin{figure}
	\centering
	\includegraphics[width=0.3\linewidth,angle=90]{img/safety-no}
	\caption{Absolute motion safety is not achievable (or undesirable) in \acl*{AD}}
\end{figure}
