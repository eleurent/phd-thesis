...
But is this an appropriate performance measure? Consider a driving decision --say merging into a roundabout as illustrated in \Cref{fig:risky-problem}-- that, given the uncertainty on the behaviour of other agents, can lead to a collision with a certain probability $\delta$. When maximising the return in expectation, the agent is allowed balance the (rate) accident penalty with the more likely perspective of a successful merge. A first observation is made by \citet{ShalevShwartz2017}: assuming that rewards in normalized in $[0, 1]$ for standard behaviours, for the agent to care about avoiding accidents with probability $1-\delta$ requires the penalty associated with a collision to be in order of $1/\delta$, as we illustrate in \Cref{tab:risky-problem}. This leads to a high variance in the observed rewards, which is likely to impede learning. And even then, the optimal agent can be willing to risk a collision once in a while, if it allows for an increased efficiency the rest of the time.

The question of whether this weighing of human lives and economical benefits is desirable brings us straight back to the ethical considerations of \Cref{sec:trolley}. Such questions often arise, as described by [ref needed]. Another recent example is the \emph{optimal} duration of COVID-19 lockdown policies, which has opposite implications for public health and economics [ref needed].
This questioning has lead researchers and practitioners to consider alternative definitions of optimality, especially tailored for safety-critical domains. These approaches are grouped under the name of \emph{Safe} \acl*{RL}, and typically involve giving up some  expected performance in favour of the lower tail of the return distribution. They were surveyed by \citet{Garcia2015}, who identified four main paradigms: the first three involve a change of the optimisation criterion, while the fourth one defines a constraint on the exploration process.

\paragraph{Risk-sensitive Criterion}

A first variation of the traditional expected return criterion consists in adding a term that penalizes the \emph{variance} of the return. For instance, in the above example of \Cref{tab:risky-problem} the two decisions are equivalent in terms of expected return, but \texttt{Merge} has a significantly higher variance than \texttt{Wait}. The variance-penalized criterion takes the form

\begin{equation*}
\max_\pi \expectedvalue_{\pi, P} \left[G_P^\pi\right] - \alpha\mathop{\mathbb{V}}_{\pi, P} \left[G_P^\pi\right],
\end{equation*}

where $\alpha > 0$ is a variance penalty coefficient, and would favour \texttt{Wait} over \texttt{Merge}.

Other measures of the risk can be considered, such as quantiles,  VaR, CVaR.
Optimisation of risk measures has been studied in the \acl*{MAB} setting, \eg by \citet{Torossian19a} for risk optimisation of stochastic blackbox functions.
[ref needed in RL.]
In the context of \acl*{AD}, \citet{Naghshvar2018} propose to optimise a variance-penalised objective with a tree-based planner and estimate variance of the return thanks to the unscented transform. They apply it to highway ramp scenarios where uncertainty stems from occlusions and limited sensor range.
...
Safety with respect to error states

We can define the concept of \emph{inevitable collision states} (ICS)
\includegraphics[height=0.4\textheight]{img/ics}
	All obstacles must be considered \emph{globally}.
\includegraphics[height=0.4\textheight]{img/ics-global}
\citep{Fraichard2004}
		
	
Absolute motion safety is not achievable
We cannot always avoid this region of the state-space.
\includegraphics[height=0.3\textheight]{img/safety-no}
But we can consider weaker models of safety:

\begin{itemize}
\item Passive motion safety (Braking ICS)\citep{Bouraine2012}
\item Passive-friendly motion safety
\item Responsibility-Sensitive Safety \citep{ShalevShwartz2017}
\end{itemize}
