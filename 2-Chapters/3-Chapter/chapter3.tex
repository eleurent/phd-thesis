...
As discussed in \Cref{sec:irl}, choosing an appropriate reward function that yields realistic optimal driving behaviour is a challenging problem, that we do not address in this thesis. Instead, we will specify a reward function as simple and straightforward as possible, so as to focus solely on the difficulties related to safe decision-making under uncertainty. Thus, we focus on only two features: a vehicle should
\begin{enumerate*}[label=(\roman*)]
...
Note that we forbid negative rewards, since they may incentivize the agent to prefer terminating an episode early (by causing a collision) rather than risking suffering a negative return if no satisfying trajectory can be found.
...
A more realistic reward function should include comfort terms, such as penalizing high acceleration or jerk, and lane changes manoeuvres, but we do not consider them for simplicity.
...
This chapter defines a class of \acp*{MDP}.

For better reproducibility, I created the \textsc{highway-env} simulator, written in Python and published online under an open-source license \citep{highway-env}. An extensive documentation is also available online, along with instructions to reproduce every numerical experiments presented throughout this manuscript.
We mention that, in addition to our own works, several researchers already make use of this environment, such as. 
Figure 4 shows the current (01/30/2020) activity status of the hosting repository.

We discuss at length its features and architecture in \Cref{chapter:a}.
