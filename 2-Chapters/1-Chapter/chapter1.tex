...
In the light of the above, this thesis is dedicated to Behavioural Planning, a weak link in the Autonomous Driving pipeline.  We ask the following question: assume that we had access to a ground truth perception and a perfectly accurate control system, what steps would remain to achieve fully autonomous driving?
%Leverage learning to design driving policies, and hope for a better comprehensiveness.


\paragraph{Humans in the loop}
% Without humans, the problem is solved
% 1st objection: humans -> hard to model -> learn from data

It is safe to assume that, in the absence of dedicated self-driving infrastructure, Autonomous Vehicles will have to share the road with human drivers at some point. 
This introduces a great deal of uncertainty in the decision problem. Indeed, while the location and velocity of a vehicle can be perceived, the mind of its driver remains impenetrable. Even though the present state is known, the future becomes uncertain: where are they headed? Are they paying attention to their surroundings? In that respect, it seems impossible to manually model all the factors involved in the human decision-making process. Yet, human drivers do not drive erratically either. Their behaviour is highly structured: humans drivers tend to follow the lanes, avoid collisions with other vehicles, and generally respect road signage. In other words, human drivers are \emph{predictable}. This motivates the idea of learning from data.%, either direct predictive models of human behaviours, or a driving policy based on implicit predictions of what they might do next.
...
Several performance measures have been introduced to evaluate \ac*{RL} algorithms. The \ac{PAC} Policy objective aims at finding a near-optimal policy $\pi^*$ with high probability in the smallest possible number of interactions (sample complexity). Likewise, \textit{simple regret} minimisation aims at minimizing the \emph{expected} sub-optimality of the recommenced policy after a fixed number $n$ of interactions. 

\begin{equation*}
r_n(s, \cA) = \expectedvalue_{\hat{\pi}_n\sim \cA} \left[ V^*(s) - V^{\hat{\pi}}(s) \right]
\end{equation*}
This is particularly relevant when exploration is not costly, for instance when a simulator is available. Conversely, in real environments where exploration is costly, another objective is to minimise the \textit{cumulative regret}.

\begin{equation*}
\cR_n(s_0, \cA) = \expectedvalue_{\hat{\pi}_t\sim \cA}\left[ \sum_{t=0}^{n-1} V^*(s_t) - V^{\hat{\pi}_t}(s_t) \right]
\end{equation*}
\TODO{Define Cumulative regret. Is this useful?}


\paragraph{Model-free \vs model-based}

In order to find an optimal policy $\pi^*$, existing \ac*{RL} algorithms can be grouped into two main families. 
\emph{Model-free} \ac*{RL} algorithms aim to optimise a policy $\pi$ directly. This often involves an \emph{evaluation} step, where the state value function $V^\pi$, or state-action value function
\begin{equation*}
Q^\pi(s,a) = \expectedvalue_{\pi, P} \left[\sum_t \gamma^t r_t\condbar s_0 = s, a_0 = a\right]
\end{equation*}
are estimated, and an \emph{improvement} step, where the policy $\pi$ is update to maximise $V^\pi$.

\emph{Model-based} \ac*{RL} algorithms attempt to estimate the \ac*{MDP} parameters $\hat{P}$ and $\hat{R}$ based on a history of transitions $\cD = \{(s_{t},a_t, r_t, s_{t+1})\}$, using \eg \ac{MLE}.
\begin{equation*}
