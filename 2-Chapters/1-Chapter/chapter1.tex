...
To find an optimal policy $\optimalpolicy$, \gls{model-based} \glsxtrlong{RL} algorithms first attempt to estimate the \gls{MDP} parameters $\hat{P}$ and $\hat{R}$ based on a history of transitions $\cD = \{(s_{t},a_t, r_t, s_{t+1})\}$, using for instance \gls{MLE} in a hypothesis class of dynamics and reward distributions.
\begin{equation*}
...
% Part III
\Cref{part:3} is devoted to the study of \gls{model-based} methods, that solve the \glsxtrlong{RL} problem by planning with a learned generative model.
