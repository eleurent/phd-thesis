...
\begin{definition}[Policy return]
	...
	The {return} $\return$ of a policy $\policy$ is a random variable defined as the discounted sum of rewards 
	\begin{equation*}
	\gls+{return} = \sum_{t=0}^\infty \discount^t \R(s_t, a_t)
	\end{equation*}
...
\begin{definition}[Value functions]
	...
	\begin{align*}
	\gls+{V}^\policy(s) \eqdef \expectedvalue\left[ \return \mid s_0=s\right].
	\end{align*}
	...
	\begin{align*}
	\gls+{Q}^\policy(s, a) &\eqdef \expectedvalue\left[ \return \mid s_0=s, a_0 = a)\right].
	\end{align*}
	\end{leftbar}
\end{definition}
...
\begin{definition}[Optimality]
	...
	We also define the optimal value functions $V^\star$ and $Q^\star$ as 
	\begin{align*}
	&\forall s\in \cS, & V^\star(s) &\eqdef Q^{\policy^\star}(s) = \max_\policy V^\policy(s);\\
	&\forall (s,a)\in \cS\times\cA,& Q^\star(s, a) &\eqdef Q^{\policy^\star}(s, a) = \max_\policy Q^\policy(s, a).
	\end{align*}
	\end{leftbar}
\end{definition}
...
\begin{equation*}
r_n(s) \eqdef \expectedvalue_{\hat{\policy}_n} \left[ V^\star(s) - V^{\hat{\policy}_n}(s) \right].
\end{equation*}
...
To find an optimal policy $\policy^\star$, \emph{model-based} \Glsxtrlong{RL} algorithms first attempt to estimate the \gls{MDP} parameters $\hat{P}$ and $\hat{R}$ based on a history of transitions $\cD = \{(s_{t},a_t, r_t, s_{t+1})\}$, using for instance \gls{MLE} in a hypothesis class of dynamics and reward distributions.
\begin{equation*}
...
% Part III
\Cref{part:3} is devoted to the study of model-based methods, that solve the \glsxtrlong{RL} problem by planning with a learned generative model.
