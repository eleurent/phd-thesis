...
\begin{figure}[ht]
	...
	\caption{Online planning with a generative model. The true interaction cycle between the agent and the environment is depicted \hlb{in blue}. At each step of real interaction, a full planning cycle of simulated trajectories is run, depicted \hlg{in green}: a look-ahead tree rooted at the current state is progressively expanded by sampling trajectories through calls to the generative model, before returning a recommendation for the estimated best action.}
