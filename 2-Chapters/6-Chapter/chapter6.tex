...
\end{enumerate*}
In this chapter, we look into the planning step in our setting where the state space $\cS$ is continuous, the action space $\cA$ is discrete. Since this precludes resorting to \glsxtrlong{DP}, we turn instead to tree-based planning algorithms, and study their performance guarantees under limited budget. First, in order to handle uncertain human behaviours modelled as \emph{stochastic} dynamics $\Ps$, we consider the \OLOP algorithm. We highlight a faulty behaviour of \OLOP in the low-budget regime, and propose a modified version that alleviates this issue. Second, we address a paradox: despite the underlying MDP transitions having a graph structure, \gls{MCTS} algorithms rely on a tree structure to represent their value estimates. Thus, we propose and analyse a \emph{graph-based} planning algorithm, that takes into account the similarity of two states reached via different trajectories. We show that this allows us to better exploits the structure of the dynamics of motion planning problems, where trajectories tend to overlap.
\minitocStartChapter{}

\section{Motivation}

In this chapter, we assume that an estimation algorithm provides us with a good estimate of the \gls{MDP} $((\cS, \cA, \hat{\Ps}, \reward)$, and consider the question of how to compute the corresponding optimal policy $\policy^\star$.

In \emph{Tree-based Planning} \citep{Munos2014}, we do not consider that these transition and reward functions are fully known as in \emph{Dynamic Programming} \citep{Bellman1957}, but rather only assume access to the \gls{MDP} through a \emph{generative model} (e.g. a simulator) which yields samples of the next state $s' \sim \probability{s'|s,a}$ and reward $r\sim\probability{r|s, a}$ when queried. 
...
\begin{proof}
	...
	\end{equation*}
	\diff{And thus, for all $b\in A^{\ast}$, by definition of $U^{\mu}$ and $L^{\mu}$:}
	\begin{align*}
