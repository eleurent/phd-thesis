...
We provide a time and memory efficient implementation of \OLOP and \KLOLOP in \Cref{sec:kl-olop-time}, bringing an exponential speed-up that allows scaling these algorithms to high sample budgets.
...
\end{lemma}
\begin{proof}
	We provide the proof in \Cref{sec:proof-lem-expected-regret}.
\end{proof}
...
\end{lemma}
\begin{proof}
	We provide the proof in \Cref{sec:proof-size_Ph}.
\end{proof}
...
\end{lemma}
\begin{proof}
	We provide the proof in \Cref{sec:proof-expected_P_size}.
\end{proof}
...
\end{lemma}
\begin{proof}
	We provide the proof in \Cref{sec:proof-expected-plays-count}.
\end{proof}
...
We have performed some numerical experiments to evaluate and compare the following planning algorithms\footnote[1]{The source code is available at \href{https://eleurent.github.io/kl-olop/}{https://eleurent.github.io/kl-olop/}.}:
\begin{itemize}
	...
	\item \OLOP: as described in \Cref{sec:kl-olop}.\footnote[2]{Note that we use the lazy version of $\OLOP$ and $\KLOLOP$ presented in \Cref{sec:kl-olop-time}, otherwise the exponential running-time would have been prohibitive.}
	\item \KLOLOP: as described in \Cref{sec:kl-olop}.\footnotemark[2]
...
In this section, we compare two data structures for planning in an MDP: tree and (directed) graph, represented in \Cref{fig:structures}. In order to distinguish them, we refer to trees with Roman symbols, \eg $T, U, L, B$; and to graphs with calligraphic symbols, \eg $\cG, \cU, \cL, \cB$. In both structures, we say that a node is \emph{internal} if it has outgoing edges, and \emph{external} else.
...
\begin{figure}[tp]
	...
	\caption{Black arrows depict how the Bellman backup operators $B_n$ (left) and $\cB_n$ (right) propagate value estimates from successor nodes to their parents. Information travels freely in a graph, but only upwards in a tree.}
...
\begin{definition}[Value bounds]
	\begin{leftbar}[defnbar]
	\leavevmode\newline
	\indent {\textbf{On trees.}} We denote by $L:\Tau_n \rightarrow \Real$ and  $U:\Tau_n \rightarrow \Real$ a lower-bound and upper-bound for the state value $V$ defined on the tree $\Tau_n$, such that
	\begin{equation*}
...
\end{lemma}
\begin{proof}
	We provide a proof in \Cref{sec:proof-properties-b-tree}.
\end{proof}
...
The corresponding \OPD algorithm is described in \Cref{alg:opd}.
...
\end{lemma}
\begin{proof}
	We provide a proof in \Cref{sec:proof-properties-b-graph}.
\end{proof}
...
\end{lemma}
\begin{proof}
	We provide a proof in \Cref{sec:proof-sequence-values}.
\end{proof}
...
\end{lemma}
\begin{proof}
	We provide a proof in \Cref{sec:proof-shrink}.
\end{proof}
...
\end{theorem}
\begin{proof}
	We provide a proof in \Cref{sec:proof-regret-bound-U}.
\end{proof}
...
\end{lemma}
\begin{proof}
	We provide a proof in \Cref{sec:proof-equivalence}.
\end{proof}
...
\end{lemma}
\begin{proof}
	We provide a proof in \Cref{sec:proof-expansion-bound}.
\end{proof}
...
\end{lemma}
\begin{proof}
	We provide a proof in \Cref{sec:proof-recommendation-bound}.
\end{proof}
...
\end{theorem}
\begin{proof}
	We provide a proof in \Cref{sec:proof-regret-gbop}.
\end{proof}
...
\end{proposition}
\begin{proof}
	We provide a proof in \Cref{sec:proof-illustrative-example}.
\end{proof}
...
%\FloatBarrier
%\begin{subappendices}
%\input{2-Chapters/6-Chapter/appendix6}
%\end{subappendices}
